...
<li class="breadcrumb-item active" aria-current="page"><a href="{{ route('annuncio.confirmDestroy',['id'=>$annuncio->id]) }}">{{ trans('labels.rimuoviAnnuncio') }}</a></li>
@endsection
...
<div class=" d-flex align-items-center justify-content-center text-center">
        <div class="col-md-6 ">
            <div class="alert alert-warning" role="alert">
                ...
                <p>{{ trans('labels.confermaRimozione') }} {{ $annuncio->posizione }}?</p>
                <hr>
                <p><a type="button" class="btn btn-outline-primary" href="{{route('annuncio.destroy', ['id' => $annuncio->id])}}"><i class="bi bi-trash3"></i> {{ trans('labels.si') }}</a>
                <a type="button" class="btn btn-outline-primary" href="{{ route('paginaAzienda.index') }}"><i class="bi bi-box-arrow-left"></i> No</a> </p>
            </div>
        </div>
    </div>
@endsection